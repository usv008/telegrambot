<script>
    ...
    var button = document.querySelector('.addProductClass');
    var disabled = false;
    // const canvas = document.getElementById('canvas');
    const canvas = document.querySelector('.convasClass');
    ...
    function setupButtons() {
        ...
        var buttons = document.querySelectorAll('.addProductClass');
        buttons.forEach((button_confetti) => {
            textElements = button_confetti.querySelectorAll('.button-text');
    ...
    $(document).ready( function() {
        ...
        $('.addProductClass').on('click', function () {
            if (!disabled) {
                ...
                this_button.classList.add('loading');
                this_button.classList.remove('ready');
                $('#addProduct_price_text').html('');
                $.ajax({
                    type: "POST",
                    url: "{{ route('add_product_to_cart') }}",
                    data: "_token={{ csrf_token() }}&user_id="+`${tg.initDataUnsafe.user.id}`+"&product_id="+this_button.getAttribute("data-product-id")+"&combination_id="+this_button.getAttribute('data-combination-id')+"&price="+this_button.getAttribute('data-price'),
                    cache: false
                }).done(function(data) {
                    ...
                    setTimeout(() => {
                        window.initBurst();
                        setTimeout(() => {
                            $('#addProduct'+this_button.getAttribute("data-product-id")).attr('data-price-all', this_button.getAttribute('data-price'));
                            this_button.classList.remove('complete');
                            this_button.classList.add('ready');
                            setTimeout(() => {
                                $('#addProduct_price_text'+this_button.getAttribute("data-product-id")).html('додати за '+$('#addProduct'+this_button.getAttribute("data-product-id")).attr('data-price')+' грн');
                                price = $('#addProduct'+this_button.getAttribute("data-product-id")).attr('data-price');
                                disabled = false;
                            }, 400);
                        }, 2700);
                    }, 500);
