...
class BotRaffleController extends Controller
{
    ...
    public static function getRaffleTry($user_id)
    {
        ...
        if (BotRaffleUsers::where('user_id', $user_id)->count() == 0) {
            ...
            $raffle_users->raffle_try = $raffle_try;
    ...
    public static function getAllRaffleTry($user_id) {
        ...
        return $raffle['raffle_try'] + $raffle['raffle_try_guest'];
    ...
    public static function getRaffleTryToday($user_id) {
        ...
        $date = date("Y-m-d");
        return BotRaffle::where('user_id', $user_id)->where('date_reg', 'like', $date.'%')->count();
    ...
    public static function getTextChances($user_id, $n) {
        ...
        $text = BotTextsController::getText($user_id, 'Raffle', 'chances_text3');

        if (
            $n == 1
            || $n == 21
        ) $text = BotTextsController::getText($user_id, 'Raffle', 'chances_text1');
        elseif (
            $n == 2
            || $n == 3
            || $n == 4
            ...
            || $n == 23
            || $n == 24
        ) $text = BotTextsController::getText($user_id, 'Raffle', 'chances_text2');
//        elseif (
//            $n == 0
//            || $n == 5
//            || $n == 6
...
//        ) $text = 'шансов';
    ...
    public static function getTextAtempts($user_id, $n) {
        ...
        $text = BotTextsController::getText($user_id, 'Raffle', 'attempts_text3');

        if (
            $n == 1
            || $n == 21
        ) $text = BotTextsController::getText($user_id, 'Raffle', 'attempts_text1');
        elseif (
            $n == 2
            || $n == 3
            || $n == 4
            ...
            || $n == 23
            || $n == 24
        ) $text = BotTextsController::getText($user_id, 'Raffle', 'attempts_text2');
    ...
    public static function updateRaffleMessageId($user_id, $raffle_id, $message_id) {
        ...
        return BotRaffle::where('id', $raffle_id)->where('user_id', $user_id)->update(['message_id' => $message_id]);
    ...
    public static function updateRaffleTry($user_id, $act) {
        ...
        if ($act == 'minus') {
            ...
            if ($raffle_user['raffle_try'] > 0) {

                $raffle_try = $raffle_user['raffle_try'] - 1;
                return BotRaffleUsers::where('id', $raffle_user['id'])->where('user_id', $user_id)->update(['raffle_try' => $raffle_try]);
        ...
        elseif ($act == 'update') {
            ...
            if ($raffle_user && $raffle_user->raffle_try == 0) {
                ...
                $raffle_try = 2;
                BotRaffleUsers::where('id', $raffle_user->id)->where('user_id', $user_id)->update(['raffle_try' => $raffle_try]);
    ...
    public static function selectBlock($user_id, $raffle_id, $p) {
        ...
        if ($block == 0) {
            ...
            if ($raffle_try < 8){
                ...
                $raffle_try++;
                BotRaffle::where('id', $raffle_id)->where('user_id', $user_id)->update(['p'.$p => '___'.$raffle_p['p'.$p], 'raffle_try' => $raffle_try]);
    ...
    public static function updateRaffleWin($user_id, $raffle_id) {
        ...
        $update_raffle = BotRaffle::where('id', $raffle_id)->where('user_id', $user_id)->update(['win' => 1]);
    ...
    public static function updateRaffleUsersWin($user_id) {
        ...
        $update_raffle_user = BotRaffleUsers::where('user_id', $user_id)->update(['win' => 1]);
    ...
    public static function checkUserRaffleWin($user_id) {
        ...
        return BotRaffleUsers::where('user_id', $user_id)->where('win', 1)->count();

    }

    public static function checkUserActionPizzaInCart($user_id) {
    ...
    public static function checkUserWin($user_id) {
        ...
        $in_cart = self::checkUserActionPizzaInCart($user_id);
    ...
    public static function addActionPizzaToCart($user_id, $product_id, $variant_id) {

        BotRaffleUsers::where('user_id', $user_id)->update(['win' => 0]);

//        $date_z = date("Y-m-d H:i:s");
//        $product_id = 43;
//        $variant_id = 53;

//        $simpla_product = Simpla_Products::getProduct($product_id);
//        $lang = BotUserSettingsController::getLang($user_id);
//        $name = 'name_'.$lang;
//        $product_name = isset($simpla_product->$name) && $simpla_product->$name !== null && $simpla_product->$name !== '' ? $simpla_product->$name : 'Піца "Неаполь" подарункова';
//
//        $cart = new BotCart;
...
//        $cart->save();
        ...
        if ($product_attribute) {
            ...
            $cart->product_action = 1;
            $cart->save();
        }
    }

    public static function updateTryForGameToday($user_id) {

        $check = self::getRaffleTryToday($user_id);
        if ($check == 0) self::updateRaffleTry($user_id, 'update');
...
//        }
    ...
    public static function updateTestRaffleMessageId($user_id, $raffle_id, $message_id) {
        ...
        return BotRaffleTest::where('id', $raffle_id)->where('user_id', $user_id)->update(['message_id' => $message_id]);
    ...
    public static function selectTestBlock($user_id, $raffle_id, $p) {
        ...
        if ($block == 0) {
            ...
            if ($raffle_try < 8){
                ...
                $raffle_try++;
                BotRaffleTest::where('id', $raffle_id)->where('user_id', $user_id)->update(['p'.$p => '___'.$raffle_p['p'.$p], 'raffle_try' => $raffle_try]);
                if ($raffle_p['p'.$p] == '🍕') $guessed++;
                return [$guessed, $raffle_try, $raffle_p['p'.$p]];
    ...
    public static function get_image ($n) {
        ...
        $image = BotSettingsRaffleImages::where('n', $n)->first()->image;
//        Log::info($image);
        return $image;
