...
class PrestaShopWebserviceController extends BaseController
{
    ...
    */
     ...
     *
     * @throws PrestaShopWebserviceExceptionController if curl is not loaded
     */
    function __construct($url, $key, $debug = false)
    {
        if (!extension_loaded('curl')) {
            throw new PrestaShopWebserviceExceptionController(
     ...
     *
     * @throws PrestaShopWebserviceExceptionController if HTTP status code is not 200 or 201
     */
    protected function checkStatusCode($request)
    {
        ...
        if (!empty($error_message)) {
            ...
            throw new PrestaShopWebserviceExceptionController(sprintf($error_label, $request['status_code'], $error_message));
        }
    }
     ...
     *
     * @throws PrestaShopWebserviceExceptionController
     */
    protected function executeRequest($url, $curl_params = array())
    {
        ...
        if ($index === false && $curl_params[CURLOPT_CUSTOMREQUEST] != 'HEAD') {
            throw new PrestaShopWebserviceExceptionController('Bad HTTP response ' . $response . curl_error($session));
        }
        ...
        if (array_key_exists('PSWS-Version', $headerArray)) {
            ...
            if (
                version_compare(PrestaShopWebserviceController::psCompatibleVersionsMin, $headerArray['PSWS-Version']) == 1 ||
                version_compare(PrestaShopWebserviceController::psCompatibleVersionsMax, $headerArray['PSWS-Version']) == -1
            ) {
                throw new PrestaShopWebserviceExceptionController(
        ...
        if ($status_code === 0) {
            throw new PrestaShopWebserviceExceptionController('CURL Error: ' . curl_error($session));
        }
        curl_close($session);
        if ($this->debug) {
    ...
    protected function parseXML($response)
    {
        if ($response != '') {
            ...
            if (libxml_get_errors()) {
                ...
                libxml_clear_errors();
                throw new PrestaShopWebserviceExceptionController('HTTP XML response is not parsable: ' . $msg);
            }
            return $xml;
        } else {
            throw new PrestaShopWebserviceExceptionController('HTTP response is empty');
        }
    }
...
//        Log::warning('PrestaShop WebService:');
//        Log::warning($request);
     ...
     * @throws PrestaShopWebserviceExceptionController
     */
    public function get($options, $json = false)
    {
        ...
        return $json ? $request['response'] : $this->parseXML($request['response']);
    }
